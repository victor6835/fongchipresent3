...
class WebpresentContentTest(unittest.TestCase):
    ...
    def test_page_numbers_are_created_for_every_slide(self):
        loop = re.search(
            r"slides\.forEach\(\(s,i\)=>\{([\s\S]*?)\n\}\);",
            self.source,
        )
        self.assertIsNotNone(loop)
        self.assertNotIn("if(i===0)return", loop.group(1).replace(" ", ""))
        self.assertRegex(loop.group(1), r"p\.textContent=\(i\+1\)\+' / '\+slides\.length")
    ...
    def test_navigation_buttons_are_centered_on_opposite_screen_edges(self):
        ...
        self.assertRegex(self.source, r"#btnPrev\s*\{[^}]*left\s*:\s*26px")
        self.assertRegex(self.source, r"#btnNext\s*\{[^}]*right\s*:\s*26px")
